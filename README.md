...
1. **Autenticación** — generar un token de acceso personal en GitHub antes de empezar (sin esto, Positron no puede hacer `commit` ni `push`).
2. **El modelo de tres estados** — cómo se mueve un archivo entre edición, staging y commit, y por qué `pull` va siempre antes de trabajar.
3. **Pull, edición, commit, push** — el ciclo básico, desde Positron (recomendado) y como alternativa desde la terminal.
4. **Conflictos** — cómo se ve un conflicto real y cómo resolverlo en el editor (Accept Current / Incoming / Both).
5. **Ramas y pull requests** — convención del grupo (una rama por persona y por tarea, nombrada `tu-nombre/descripción-corta`) y cómo proteger `main` en GitHub.
6. **Codex en Positron** — generación y corrección de código, y qué modelo usar según la tarea (liviano para explorar, especializado en código para generar o corregir).
7. **Resolución de problemas frecuentes** — los errores más comunes del flujo y cómo resolverlos.
...
- `practica/conflicto-positron.svg`, `practica/crear-rama-positron.svg` — ilustraciones usadas en la guía.

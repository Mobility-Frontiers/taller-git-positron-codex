...
Lo primero, antes de clonar nada: Positron necesita saber con qué cuenta de GitHub va a trabajar. El ícono de cuenta está en la esquina inferior izquierda de la barra de actividad — al hacer clic ahí aparece la opción para iniciar sesión:

![Ícono de cuenta en la barra de actividad de Positron, con la opción "Sign in with GitHub to use Git and GitHub features".](practica/positron-cuenta-github.svg)

Al elegir esa opción, Positron pide confirmar en el navegador: se abre una pestaña nueva, se ingresa un código de un solo uso, y se autoriza el acceso. Desde ese momento, Positron queda conectado con esa cuenta para todo lo que sigue — clonar, hacer pull, push, y abrir pull requests sin tener que autenticarse de nuevo cada vez:

![Aviso de Positron pidiendo iniciar sesión con GitHub, con el botón Allow y el código que se confirma en el navegador.](practica/positron-signin-github.svg)
...
Con la cuenta ya conectada, cada persona necesita su propia copia local del repositorio. Desde Positron:

1. Paleta de comandos (`Cmd/Ctrl+Shift+P`) → **Git: Clone**.
2. Elegir **Clone from GitHub** y buscar `taller-git-positron-codex`, o **Clone from URL** y pegar la dirección directamente.
...
Al terminar, Positron pregunta si se quiere abrir la carpeta — elegir **Open**.

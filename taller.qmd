...
format:
  html:
    ...
    include-after-body:
      text: |
        <button id="gtranslate-btn" class="gtranslate-btn" type="button">Translate to English</button>
        ...
        function gtranslateToggle() {
          var btn = document.getElementById('gtranslate-btn');
          var alreadyTranslated = document.documentElement.classList.contains('translated-ltr');
          ...
          if (combo) {
            ...
            btn.textContent = 'Cargando traductor…';
            setTimeout(gtranslateToggle, 600);
          }
        }
        document.getElementById('gtranslate-btn').addEventListener('click', gtranslateToggle);
        </script>
execute:
  echo: false
  eval: false
...
## Conectar tu cuenta de GitHub con Positron
...
![Menú de cuenta en Positron una vez conectado: el nombre de usuario de GitHub, con acceso a Manage Trusted Extensions, Manage Trusted MCP Servers y Sign Out.](practica/positron-cuenta-github.svg)
...
- [ ] Abrir el ícono de cuenta en la esquina inferior izquierda de Positron.
- [ ] Elegir "Sign in with GitHub" y confirmar en el navegador.
- [ ] Verificar que el nombre de usuario de GitHub aparece en ese mismo panel.
:::
...
1. Paleta de comandos (`Cmd/Ctrl+Shift+P`) → **Workspaces: New Folder from Git…**.
2. Elegir **Clone from GitHub** y buscar `taller-git-positron-codex`, o pegar la URL directamente si aparece esa opción.
3. Elegir la carpeta local donde va a quedar guardada la copia.

![Clonar el repositorio desde Positron: comando Workspaces: New Folder from Git, URL del repositorio, y selección de la carpeta local de destino.](practica/clonar-repo-positron.svg)

Al terminar, Positron pregunta si se quiere abrir la carpeta — elegir **Open**. También va a pedir confirmar que se confía en el repositorio (Workspace Trust) — al ser un repositorio del propio grupo, se confirma.

::: {.exercise}
**Ejercicio 2 — Clonar el repositorio de práctica**

- [ ] Clonar `taller-git-positron-codex` desde Positron (Workspaces: New Folder from Git…).
- [ ] Abrir la carpeta clonada en Positron y confirmar que se confía en el repositorio.
:::
...
Con el token disponible, la primera vez que la terminal pida credenciales al ejecutar `git push`:
...
- [ ] Tenerlo a mano por si la terminal lo pide como contraseña más adelante.
:::
...
![Vista Pull Requests de Positron: botón Create Pull Request y el formulario con rama base, rama de comparación, título y descripción.](practica/github-crear-pr.svg)
...
![Revisión de un pull request dentro de Positron: el diff del archivo, un comentario de otra persona del grupo, la aprobación, y el botón Merge Pull Request.](practica/github-revisar-pr.svg)
...
- [ ] Desde la vista Pull Requests de Positron, crear el PR hacia `main` (botón Create Pull Request).

...
Con `taller-git-positron-codex` abierto en Positron, el panel de control de código fuente (ícono de rama en la barra lateral izquierda) concentra la mayor parte de las acciones. Este es el método recomendado para el taller: reduce la posibilidad de error respecto a escribir los comandos manualmente, y permite ver el estado del repositorio en todo momento.
...
Ambos métodos producen exactamente el mismo resultado; el panel de Positron es una interfaz gráfica sobre estos mismos comandos. Para el taller se recomienda el panel gráfico, y se deja la terminal como referencia para quienes ya estén familiarizados con la línea de comandos.
...
**Desde Positron (recomendado):** el editor presenta este mismo bloque con opciones sobre cada sección en conflicto — *Accept Current*, *Accept Incoming*, *Accept Both* — para seleccionar la versión que corresponde, o combinar ambas manualmente. La siguiente imagen reproduce cómo se ve exactamente este conflicto dentro del editor:
...
2. El nombre de la rama sigue el formato `nombre/descripción-corta` — por ejemplo, `roberto/limpieza-datos-gps` o `camila/modelo-difusion-v2`. El nombre identifica de inmediato quién está trabajando en qué, algo especialmente útil cuando varias personas tienen ramas abiertas al mismo tiempo.
...
**Desde Positron (recomendado):** el nombre de la rama actual aparece en la esquina inferior izquierda de la ventana. Al hacer clic ahí, Positron ofrece la opción de crear una rama nueva a partir de un nombre. El ciclo de edición, commit y push se ejecuta después de manera idéntica al descrito en el ejercicio anterior; la única diferencia es que los cambios quedan aislados en esa rama.
...
::: {.exercise}
**Ejercicio 4 — Crear tu rama siguiendo la convención del grupo**
